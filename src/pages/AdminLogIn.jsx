...
import { adminUrl, memberUrl } from "../url";
...
function AdminLogIn(){
    ...
    function logIn(){
        API.post(`${adminUrl}/login`, toFormData(input)).then((data)=>{
            if(data.status == true){
    ...
    return(
        <div>
            ...
            <div className=" w-[600px] h-auto pb-[50px] mb-20 mt-10 mx-auto p-4 rounded-sm bg-slate-200 relative">
                <h3 className=" py-2 text-xl text-center font-bold text-sky-900">Admin LogIn</h3>
                <div>
                ...
                <h4 className=" text-sky-900 mt-20">
                    {/* Don’t Have An Account? Please, <Link to="/memberSignUP" className=" text-yellow-600 underline"> Register</Link> Here */}
                    </h4>
            </div>
        </div>
    )
